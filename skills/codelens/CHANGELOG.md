...
### Fixed
- **refactor-safe command broken**: Imported `is_generated_file` from `utils` which did not exist, causing the entire command to fail on every invocation. Now `is_generated_file` is properly implemented in `utils.py`.
- **binary-scan command broken**: Imported `scan_tauri_artifacts` from `utils` which did not exist, causing the command to fail. Now `scan_tauri_artifacts` is properly implemented in `utils.py`.
- **Smell engine massive false positives on generated files**: Files in `generated/` directories, vendor directories, protobuf files, and minified files were not skipped. Now uses `is_generated_file()` to skip auto-generated files entirely. Reduced smells from 84,458 → 57,400 on home-assistant/core.
- **Magic number detection Python false positives**: Numbers in Python docstrings, UPPER_CASE constant assignments, type annotations, default function arguments, range()/enumerate() calls, logging configuration, and assert statements were flagged as magic numbers. Now skips all these patterns. Added `constants`, `const.py`, `enums` to config file skip keywords. Expanded `common_numbers` with frequently used constants (4-9, 12-64, 128-1024, etc.).
- **Python global state overcounting**: Module-level variables that were private (leading `_`), dunder attributes (`__tablename__`, `__slots__`), class-type assignments, lambda/function/class definitions, simple string/number/boolean constants, frozenset/set/tuple/list/dict literals, and logging assignments were all counted as global state stores. Now properly filtered. Reduced from 581 → 52 stores on home-assistant/core.
- **Handbook misclassifies Python applications as libraries**: Projects with `pytest` in pyproject.toml were classified as `python-library`, even when they were full applications (e.g., Home Assistant). New `_is_python_application()` heuristic checks for Dockerfile, __main__.py, console_scripts entry point, and large component directories.

### Added
- **`is_generated_file()` utility**: New function in `utils.py` that detects auto-generated files by path patterns (generated/, vendor/, _pb2.py, .min.js, .d.ts, dist/, build/, third_party/) and content markers (DO NOT EDIT, auto-generated, generated by). Returns boolean for use by any engine.
- **`scan_tauri_artifacts()` utility**: New function in `utils.py` that scans for Tauri-specific configuration and source files. Detects tauri.conf.json, Tauri.toml, src-tauri directory, capability files, and Rust backend sources. Returns structured findings with config data.
- **Home Assistant framework detection**: New `homeassistant` entry in `FRAMEWORK_SIGNATURES` with pip package and directory indicators (homeassistant/components/, homeassistant/core.py, homeassistant/bootstrap.py). Sets `has_homeassistant` flag and adds recommended backend paths.
- **`python-application` project type**: New project identity type for Python applications that are not libraries or backend APIs. Detected by Dockerfile presence, __main__.py entry point, console_scripts in pyproject.toml, or large component/integration directories (10+ subdirs).
- **Python-aware magic value filtering**: New `python_skip_patterns` list in `_detect_magic_values()` with 20+ regex patterns that match Python constructs where numbers are not magic values.
