...
def is_generated_file(filename: str) -> bool:
    ...
    Covers lock files, minified/compiled output, declaration files,
    and common build artifact patterns.
    ...
    if filename in _GENERATED_FILE_PATTERNS:
        return True
    # Check extension-based patterns (e.g., .min.js, .d.ts)
    name_lower = filename.lower()
    for ext in _GENERATED_FILE_EXTENSIONS:
        if name_lower.endswith(ext):
            return True
    # Auto-generated header patterns
    for prefix in ('.generated.', '.auto.', '.autogenerated.'):
...
def walk_source_files(
    ...
    """Walk the workspace and yield (rel_path, ext, content) for source files.

    A centralized file walker that all engines should use instead of
    rolling their own os.walk loops. This ensures consistent ignore
    rules, size limits, and generated-file filtering.

    Args:
        workspace: Absolute path to workspace root.
        extensions: Set of file extensions to include (e.g., {'.py', '.js'}).
                   If None, includes all source-like extensions.
    ...
    if extensions is None:
        extensions = {'.py', '.js', '.mjs', '.cjs', '.ts', '.tsx', '.jsx',
                      '.rs', '.html', '.css', '.scss', '.less', '.vue',
                      '.svelte', '.php', '.blade.php', '.proto',
                      '.graphql', '.gql', '.go', '.cpp', '.c', '.h', '.hpp'}
    ...
    for root, dirs, filenames in os.walk(workspace):
        ...
        for filename in filenames:
            ...
            content = safe_read_file(file_path, max_size=max_size)
            if content is None:
                continue
