...
def is_bundled_file(rel_path: str) -> bool:
    """Check if a relative file path looks like a bundled/compiled/minified output file.

    These files are typically generated by build tools (Webpack, Vite, Rollup, etc.)
    and should be skipped during code analysis because they are not source code.

    Matches:
    - Files in dist/, build/, out/, .output/ directories
    - Minified files: *.min.js, *.min.css
    - Bundle/chunk files: *.bundle.js, *.chunk.js, *.vendor.js
    - Global CSS files: *.global.css
    - Hashed output files: app.[hash].js, chunk.[hash].css

    Args:
        rel_path: Relative file path from workspace root (e.g., 'dist/app.abc123.js')
    ...
    # Directory-based detection: files in known output directories
    bundled_dirs = {'dist/', 'build/', 'out/', '.output/', '.next/', '.nuxt/'}
    for d in bundled_dirs:
        if normalized.startswith(d) or f'/{d}' in normalized:
            return True

    # Extension-based detection: minified/bundled files
    lower = normalized.lower()
    if lower.endswith(('.min.js', '.min.css', '.min.mjs')):
        return True
    if lower.endswith(('.bundle.js', '.chunk.js', '.vendor.js')):
        return True
    if lower.endswith(('.bundle.css', '.chunk.css')):
        return True
    if lower.endswith('.global.css'):
        return True

    # Hashed output pattern: name.[8+hex chars].js/css (e.g., app.abc123de.js)
    if re.search(r'\.[a-f0-9]{8,}\.(js|mjs|css)$', lower):
        return True
