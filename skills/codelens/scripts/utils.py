...
# ─── Tauri / Electron Artifact Scanning ────────────────────

def scan_tauri_artifacts(workspace: str) -> Optional[Dict[str, Any]]:
    """Scan workspace for Tauri/Electron artifacts and configuration.

    Detects:
    - Tauri IPC commands/handlers from Rust source
    ...
    Returns:
        Dict with Tauri/Electron analysis, or None if neither detected.
    """
    workspace = os.path.abspath(workspace)

    # Check for Tauri markers
    tauri_markers = ['tauri.conf.json', 'Tauri.toml', 'src-tauri']
    has_tauri = False
    for marker in tauri_markers:
    ...
    electron_markers = ['electron-builder.yml', 'electron-builder.json', 'main.js']
    has_electron = False
    ...
    if not has_tauri and not has_electron:
        return None
    ...
    # Analyze Tauri IPC commands from Rust source
    if has_tauri:
        ...
        for root, dirs, files in os.walk(src_tauri):
            ...
            for f in files:
                ...
                try:
                    ...
                    if not content:
                        continue
                    # Detect #[tauri::command] decorated functions
                    import re
                    ...
                    # Detect invoke::handler
                    for m in re.finditer(r'\.invoke_handler\(.*?generate_handler\!\(\[(.*?)\]\)', content, re.DOTALL):
                        handlers = re.findall(r'(\w+)', m.group(1))
                        for h in handlers:
                            if h not in [c["name"] for c in result["ipc_commands"]]:
                                result["ipc_commands"].append({
                                    "name": h,
                                    "file": os.path.relpath(fpath, workspace),
        ...
        # Check tauri.conf.json for sidecars and security
        tauri_conf_paths = [
        ...
        for conf_path in tauri_conf_paths:
            ...
            try:
                ...
                for sc in external_bin:
                    result["sidecars"].append({"name": sc, "config_file": os.path.relpath(conf_path, workspace)})
                # Check security
                security = conf.get("security", {})
                csp = security.get("csp", None)
                if not csp or csp == "default-src 'self'":
                    result["security_notes"].append({
                        "type": "csp",
                        "message": "CSP is not configured or uses default policy",
                        "severity": "warning"
                    })
                asset_protocol = security.get("assetProtocol", {})
                if asset_protocol.get("enable", False):
                    result["security_notes"].append({
                        "type": "asset_protocol",
                        "message": "Asset protocol is enabled — allows access to local files",
                        "severity": "warning"
                    })
                # Check dangerous permissions
                permissions = conf.get("permissions", [])
                dangerous_perms = [p for p in permissions if "shell" in str(p).lower() or "fs" in str(p).lower()]
                if dangerous_perms:
                    result["security_notes"].append({
                        "type": "dangerous_permissions",
                        "message": f"Potentially dangerous permissions: {dangerous_perms}",
                        "severity": "high"
                    })
            except Exception:
                pass
    ...
    if has_electron:
        result["security_notes"].append({
            "type": "electron",
            "message": "Electron app detected — check for nodeIntegration and contextIsolation settings",
            "severity": "info"
        })
