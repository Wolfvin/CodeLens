...
## What's New in v6.1 — Tested on home-assistant/core (26K files, 17.5K Python, 1480 IoT integrations)

- **Broken commands fixed**: `refactor-safe` was broken (imported missing `is_generated_file` from utils) and `binary-scan` was broken (imported missing `scan_tauri_artifacts` from utils). Both now work correctly.
- **Generated file detection (`is_generated_file`)**: New utility function that detects auto-generated files by path patterns (generated/, vendor/, _pb2.py, .min.js, .d.ts, dist/, build/) and content markers (DO NOT EDIT, auto-generated, generated by). Used by smell engine and refactor-safe engine.
- **Smell engine false positive reduction (v2)**: 84,458 → 57,400 smells on home-assistant/core. Generated files now skipped entirely. Python-aware magic number detection: skips docstrings, UPPER_CASE constants, type annotations, default arguments, range()/enumerate() calls, logging config, assert statements, and common Python patterns. Expanded common_numbers set with frequently used constants. New config file keywords (constants, const.py, enums) skip entire files.
- **Python global state false positive reduction (v4)**: 581 → 52 state stores on home-assistant/core. Now skips: private variables (leading `_`), dunder attributes (`__tablename__`, `__slots__`, etc.), test directories aggressively, class-type assignments (type aliases), lambda/function/class definitions, simple string/number/boolean constants, frozenset/set/tuple/list/dict literals, logging/getLogger assignments. Expanded PY_CONSTANT_SKIP with framework patterns (DOMAIN, PLATFORM, SERVICE, etc.).
- **Framework detection: Home Assistant**: Detects `homeassistant` framework via pip dependency and directory indicators (homeassistant/components/, homeassistant/core.py). Adds recommended backend paths for component/integration directories.
- **Handbook identity: python-application type**: Projects with Dockerfile, __main__.py, console_scripts entry point, or large component/integration directories (10+ subdirs) are now classified as `python-application` instead of `python-library`. Home Assistant correctly identified as application, not library.
- **Tauri artifact scanning (`scan_tauri_artifacts`)**: New utility function that scans for Tauri-specific configuration and Rust source files. Detects tauri.conf.json, Tauri.toml, src-tauri directory, capability files, and Rust backend sources.
